...
Options:
  ...
  -m, --minutes <number>     Time window in minutes (default: 1440 = 24 hours)
...
/**
 * Format incidents for console output (table style)
 */
function formatTable(incidents) {
    ...
    // Get column widths
    const cols = {
        incident_id: Math.max('Incident ID'.length, ...incidents.map(i => String(i.incident_id).length)),
        alert_name: Math.max('Alert Name'.length, ...incidents.map(i => String(i.alert_name).length)),
        timestamp: Math.max('Timestamp'.length, ...incidents.map(i => String(i.timestamp_readable).length)),
        status: Math.max('Status'.length, ...incidents.map(i => String(i.status).length)),
        severity: Math.max('Severity'.length, ...incidents.map(i => String(i.severity).length)),
    };

    // Header
    let output = '';
    output += `${'Incident ID'.padEnd(cols.incident_id)} | `;
    output += `${'Alert Name'.padEnd(cols.alert_name)} | `;
    output += `${'Timestamp'.padEnd(cols.timestamp)} | `;
    output += `${'Status'.padEnd(cols.status)} | `;
    output += `${'Severity'.padEnd(cols.severity)}\n`;
    output += `-`.repeat(cols.incident_id + cols.alert_name + cols.timestamp + cols.status + cols.severity + 12) + '\n';

    // Rows
    incidents.forEach(incident => {
        output += `${String(incident.incident_id).padEnd(cols.incident_id)} | `;
        output += `${String(incident.alert_name).padEnd(cols.alert_name)} | `;
        output += `${String(incident.timestamp_readable).padEnd(cols.timestamp)} | `;
        output += `${String(incident.status).padEnd(cols.status)} | `;
        output += `${String(incident.severity).padEnd(cols.severity)}\n`;
    });
...
/**
 * Format incidents as CSV
 */
function formatCSV(incidents) {
    if (!incidents || incidents.length === 0) {
        return '';
    }
    ...
    const rows = incidents.map(incident =>
        headers.map(header =>
            JSON.stringify(incident[header])
        ).join(',')
    );
...
async function main() {
    ...
    // Validate parameters
    if (!params.customerId && !params.topology && !params.allCustomers) {
        console.error('\n❌ Error: Must specify --customer-id, --topology, or --all\n');
        printHelp();
        process.exit(1);
    }
    ...
    try {
        ...
        if (params.allCustomers) {
            incidents = await extractAllIncidents(params.minutes, params.excludeAlertNames, params.verbose);
        } else if (params.topology) {
            incidents = await extractIncidentsByTopology(params.topology, params.minutes, params.excludeAlertNames, params.verbose);
        } else {
            incidents = await extractIncidents(params.customerId, params.minutes, params.excludeAlertNames, params.verbose);
        }

        // Format output
        let output = '';
        if (params.output === 'json') {
            output = JSON.stringify(incidents, null, 2);
        } else if (params.output === 'csv') {
            output = formatCSV(incidents);
        } else {
            output = formatTable(incidents);
        }
